<?php
    // SQL commands used to create SQL tables
    // CREATE TABLE credentials (
    //     Email varchar(256) NOT NULL,
    //     Username varchar(256) NOT NULL UNIQUE,
    //     Password varchar(256) NOT NULL,
    //     Salt1 char(5) NOT NULL,
    //     Salt2 char(5) NOT NULL,
    //     -- We make the username the primary key because we have no reason to assume 
    //     -- it will change under the functionality available in the system
    //     PRIMARY KEY(Username)
    // );
    
    // CREATE TABLE files (
    //     Content_Name varchar(256) NOT NULL,
    //     File_Content varchar(256) NOT NULL,
...
_END;
    ...
    if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['email'])){
        $salt1 = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ!@$%^&*),./?;:[]{}|'),1,5);
        $salt2 = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ!@$%^&(),./?;:[]{}|'),1,5);
        ...
        add_user($connection, $email, $username, $token, $salt1, $salt2);
    }

    function add_user($connection, $em, $un, $pw, $salt1, $salt2) {
        $query = "INSERT INTO credentials VALUES('$em', '$un', '$pw', '$salt1', '$salt2')";
